...
double logLikPrunningREML_ext(arma::mat X, arma::uword k, arma::uword p, arma::vec nodes, arma::uvec des, arma::uvec anc, arma::uvec names_anc, arma::mat mapped_edge, arma::cube Rcorr, arma::mat sd_mat) {

  // Function to compute the likelihood of the model using pruning algorithm.
  // Very similar to the previous function. However here we conduct the reconstruction of the R matrix for each of the visited branches. Note that the sd_mat parameter is a matrix of n_trais x n_branches with the standard deviation vector for each of the branches of the tree. The correlation matrix Rcorr is shared among all branches of the tree.
  ...
  arma::uword des_node1;
  arma::vec ss = vec(k); // Number of traits.
  arma::cube Rs1 = cube(Rcorr);
  arma::cube Rs2 = cube(Rcorr);
  arma::mat R_tmp = mat(k, k);; // Used to stored the current reconstructed R matrix. (Once per branch!)
  arma::mat diag_mat_tmp = mat(k, k); // Used to store a temporary diagonal matrix.
  arma::mat Rinv = mat(k, k);
  arma::uvec des_node_vec = uvec(2);
  arma::uword nd;
  arma::uword nd_id;
  ...
  for(arma::uword i=0; i < n_nodes; i++) {
	...
	// Note the stop condition for the loop. We can use the number of regimes here because the loop will stop when i=1 < p=2 !! And NOT when j=2 !!
	// Multiply each R matrix for the correspondent branch length.
	diag_mat_tmp = diagmat( sd_mat.row(node_id[0]) );
	R_tmp = diag_mat_tmp * cov2cor_C( Rcorr.slice(j) ) * diag_mat_tmp;
	Rs1.slice(j) = R_tmp * mapped_edge(node_id[0],j);
	diag_mat_tmp = diagmat( sd_mat.row(node_id[1]) );
	R_tmp = diag_mat_tmp * cov2cor_C( Rcorr.slice(j) ) * diag_mat_tmp;	
	Rs2.slice(j) = R_tmp * mapped_edge(node_id[1],j);
      }
      // Join all slices together, sum everything into a single matrix.
      // Without copying it all again! Awesome!
      for(arma::uword z = 1; z < p; z++) {
	...
	// Note the stop condition for the loop. We can use the number of regimes here because the loop will stop when j=1 < p=2 !! And NOT when j=2 !!
	// Multiply each R matrix for the correspondent branch length.
	diag_mat_tmp = diagmat( sd_mat.row(tip_id) );
	R_tmp = diag_mat_tmp * cov2cor_C( Rcorr.slice(j) ) * diag_mat_tmp;
	Rs1.slice(j) = R_tmp * mapped_edge(tip_id,j);
	diag_mat_tmp = diagmat( sd_mat.row(nd_id) );
	R_tmp = diag_mat_tmp * cov2cor_C( Rcorr.slice(j) ) * diag_mat_tmp;	
	Rs2.slice(j) = R_tmp * mapped_edge(nd_id,j);
      }
      // Rf_PrintValue( wrap( row2 ) );
      // Join all slices together, sum everything into a single matrix.
      // Without copying it all again! Awesome!
