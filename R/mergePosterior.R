...
mergePosterior <- function(...){
    ...
    if( sum( sapply(chains, function(x) inherits(x, what=c("ratematrix_single_chain", "ratematrix_multi_chain", "ratematrix_poly_chain")) ) ) == length(chains) ){
        ...
        if( inherits(chains[[1]], what="ratematrix_single_chain") ){
            p <- 1
        }
        ...
        if( inherits(chains[[1]], what="ratematrix_poly_chain") ){
            ## In this case we could have a single or multiple rate regimes.
    ...
    ## Need to separate the classes here to not break the code.
    if( inherits(chains[[1]], what=c("ratematrix_single_chain", "ratematrix_multi_chain")) ){
    ...
    if( inherits(chains[[1]], what="ratematrix_poly_chain") ){
        ## The new MCMC class:
        ## list(matrix = RR, tip_samples = tip_samples, anc_samples = anc_samples, trait.names = out$trait.names, n_post_samples = nrow(tip_samples)        
        if( p == 1 ){
            mcmc.join$matrix <- do.call(c, lapply(chains, function(x) x$matrix) )
        } else{
            mcmc.join$matrix <- lapply(1:p, function(y) do.call(c, lapply(chains, function(x) x$matrix[[y]]) ) )
        ...
        mcmc.join$tip_samples <- do.call(rbind, lapply(chains, function(x) x$tip_samples) )
        mcmc.join$anc_samples <- do.call(rbind, lapply(chains, function(x) x$anc_samples) )
        mcmc.join$trait.names <- chains[[1]]$trait.names
        mcmc.join$n_post_samples <- nrow( mcmc.join$anc_samples ) ## Update the number of samples.
        class( mcmc.join ) <- "ratematrix_poly_chain"
    }
