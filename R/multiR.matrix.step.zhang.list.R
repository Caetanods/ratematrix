...
multi.sigma.step.zhang.list <- function(cache.data, cache.chain, prior, v, w_sd, w_mu, iter, count, n.phy) {
    ...
    cache.chain$which.phy[count] <- sample(1:n.phy, size = 1) ## The index for the phy used for the log.lik.
    ...
    if( up == 1 ){
        ...
        Rlik[[Rp]] <- prop.vcv
        prop.sd.lik <- loglikMCMC(cache.data$X, cache.data$k, cache.data$nodes[[cache.chain$which.phy[count]]]
    ...
    if( up == 2 ){
        ...
        Rlik <- cache.chain$chain[[iter-1]][[4]] ## This log lik need both matrices.
        Rlik[[Rp]] <- prop.vcv
        prop.r.lik <- loglikMCMC(cache.data$X, cache.data$k, cache.data$nodes[[cache.chain$which.phy[count]]]
                               , cache.data$des[[cache.chain$which.phy[count]]], cache.data$anc[[cache.chain$which.phy[count]]]
                               , cache.data$mapped.edge[[cache.chain$which.phy[count]]]
                               , R=Rlik, mu=as.vector( cache.chain$chain[[iter-1]][[1]] ) )
