...
##' @export
testRateMatrix <- function(mcmc.chain, out, par="all"){
    ...
    if(par == "rates"){
        for(i in ncol(comb)){
            mat1 <- t( sapply(mcmc.chain$matrix[[comb[1,i]]], function(x) c( diag(x) ) ) )
            mat2 <- t( sapply(mcmc.chain$matrix[[comb[2,i]]], function(x) c( diag(x) ) ) )
            mat.diff <- mat1 - mat2
            median.diff[[i]] <- apply(mat.diff, 1, median)
        }
    }
    if(par == "correlation"){
        ...
        if( ncol( mcmc.chain$matrix[[1]][[1]] ) > 2 ){
            upper <- upper.tri( mcmc.chain$matrix[[1]][[1]] )
            for(i in ncol(comb)){
                mat1 <- t( sapply(mcmc.chain$matrix[[comb[1,i]]], function(x) c( cov2cor(x)[upper] ) ) )
                mat2 <- t( sapply(mcmc.chain$matrix[[comb[2,i]]], function(x) c( cov2cor(x)[upper] ) ) )
                mat.diff <- mat1 - mat2
                median.diff[[i]] <- apply(mat.diff, 1, median)
            }
        } else{
