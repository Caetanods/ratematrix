...
##' @param single_rate whether the MCMC results are for a single or multiple regimes.
...
##' @importFrom ape Ntip
##' @noRd
readPolyMCMC <- function(out, burn = 0.5, thin = 1, dir=NULL, single_rate = FALSE){
    ...
    ## In this case the MCMC file just have the rates. No root values.
    ...
    ## Here we just have the rates. We don't have the root values!
    if( single_rate ){
        ## Only a single regime.
        init <- 1
        end <- out$k^2
        ## A single list with all the rate matrices.
    ...
    ## ##################################
    ## Get the samples for the tips:
    ## ##################################
    trait_space <- read_lines( file=file.path(direct, paste(out$outname, ".", out$ID, ".traitspace", sep="")) )
    trait_space_header <- trait_space[1]
    trait_space_header <- as.character( strsplit(x=trait_space_header, split="; ", fixed=TRUE)[[1]] )
    trait_space <- trait_space[post]
