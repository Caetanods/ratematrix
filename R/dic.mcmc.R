...
dic.mcmc <- function(out, post){
    ...
    if(out$p > 1){
        ...
        lik.mean.par <- multiR.loglik(data=cache.data, chain=NULL, root=root.mean, R=R.mean)
        ...
        lik.mean.par <- singleR.loglik(data=cache.data, chain=NULL, root=root.mean, R=R.mean)
