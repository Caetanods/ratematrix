...
##' @export
make.prior.barnard <- function(mu, sd, min, max){
    ...
    ##      vcv matrix.
    prior <- list(
        prior_mu = function(x) log.dmvnorm(x, mu, sigma = diag(sd^2, length(x))),
	prior_S = function(x) prior.vcv.barnard(x, min=min, max=max)
        )
    return(prior)
}
