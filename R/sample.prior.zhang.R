...
##'
##' This function is useful to generate a starting value for a MCMC chain and to plot the prior distribution for the model. When generating the starting value make sure to set n=1. When plotting the prior distribution, one need to use a larger number of samples to have a nice looking plot. For the starting value of a MCMC one can choose whether to sample the starting vector of standard deviations from its prior or to derive from the covariance matrix. If the MCMC is falling to run, changing "sample.sd" to FALSE might help the evaluation of the likelihood function. However, if you are taking samples to plot the prior, make sure to set "sample.sd" to TRUE, otherwise the plot will not reflect the set of prior densities used in the MCMC chain.
