...
Here I will take samples from the default prior distribution used in the `ratematrix` package using the function `makeStart`. This function is mostly used internally to set the initial point when runing `ratematrixMCMC` with `start = "prior_sample"`. Then, I will use the output to show examples of the format necessary to set custom starting points.
...
Here `data$phy` is a `simmap` tree with two regimes. The regimes are `island` and `mainland`. The order of the correlation matrices and vectors of variance components is the same; first the correlation and variance associated with `island` and second the one for `mainland`.
...
Now we can get two random samples from this distribution and use it to set the initial state for the second MCMC run comprising the two regimes (`island` and `mainland`).
